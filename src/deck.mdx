...
**Community**: Use something widely supported from the community
...
No need for DSL for basic statements
...
Angular slice filter. Again, special DSL:
```html
<p>{{ name | slice:1:5 }}</p>
```
...
**Productivity**: leverage JavaScript to deal with logics and code path

**Productivity**: Use standard JavaScript APIs rather than awkward DSLs
...
... but even this feature will be **disabled in Angular 6**.

---

## Angular 6 and Webpack

Angular 6 will introduce *a new builder system* which is an abstraction layer on top of many building processes including webpack.

However, this system is **not ready**. And no announcement of when it will be ready.
...
Our business requires high-level of customization and performance.
...
<Image src="assets/images/webpack-kindergarten.png" size="contain" />
